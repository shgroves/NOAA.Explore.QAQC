...
dat1 <- survey_df |>
  ...
  # Add in values from the TaxonRank column that do not have a column of their own
  dplyr::mutate(Gigaclass = NA_character_,
...
# We are looking for the second to lowest taxonomic rank, so filter out NAs
  dplyr::filter(!is.na(Taxon2)) |>
  
  
test <- 
# Create a substrate dataframe
substrate <- survey_df |>
  # Subset substrate and metadata
  dplyr::select(SurveyID, Station, Latitude, Longitude, DepthInMeters, Habitat, Substrate, CMECSGeoForm, ReporterEmail, IdentificationDate, IdentificationQualifier, ObservationDate, IndividualCount, ObservationTime, LocationAccuracy)
