...
```
**Table 15.** Rows missing sensor information: Density
```{r Missing data Density, echo=FALSE}

missing_density <- survey_df[is.na(survey_df$Density),]

if(nrow(missing_density) == 0)
{
  "No missing data"
} else
{
  if (print_table){
    missing_density %>%
    dplyr::select(SurveyID, ObservationDate,ObservationTime, SampleID)
  DT::datatable(missing_density)
  }
  else{
    print(paste( missing_density %>% nrow()   ,  "rows missing in density column"))
   
    
  }
}
```
**Table 16.** Rows missing sensor information: Cover
```{r Missing data Cover, echo=FALSE}

missing_cover <- survey_df[is.na(survey_df$Cover),]
...
} else
{
  if (print_table){
    missing_cover %>%
    dplyr::select(SurveyID, ObservationDate,ObservationTime, SampleID)
  DT::datatable(missing_cover)
  }
  else{
    print(paste(missing_cover %>% nrow()  ,  "rows missing in cover column"))
   
    
  }
}
```
**Table 17.** Rows missing geographical information: Habitat
...
```
**Table 18.** Rows missing geographical information: Substrate
...
**Table 19.** Rows missing sensor information: Temperature
...
```
**Table 20.** Rows missing sensor information: Salinity
```{r Missing data Salinity, echo=FALSE}

missing_salinity <- survey_df[is.na(survey_df$Salinity),]

if(nrow(missing_salinity) == 0)
{
  "No missing data"
} else
{
  if (print_table){
    missing_salinity %>%
    dplyr::select(SurveyID, ObservationDate,ObservationTime, SampleID)
  DT::datatable(missing_salinity)
  }
  else{
    print(paste(missing_salinity %>% nrow()  , "rows missing in salinity column"))

  }
}
```
**Table 21.** Rows missing sensor information: Oxygen
```{r Missing data Oxygen, echo=FALSE}

missing_ox <- survey_df[is.na(survey_df$Oxygen),]

missing_ox <- survey_df[is.na(survey_df$Substrate),]

if(nrow(missing_ox) == 0)
{
  "No missing data"
} else
{
  if (print_table){
    missing_ox %>%
    dplyr::select(SurveyID, ObservationDate,ObservationTime, SampleID)
  DT::datatable(missing_ox)
  }
  else{
    print(paste(missing_ox %>% nrow(), "rows missing in oxygen column"))
  }
}
```
...
# Plot Map
```{r}
plt_df = survey_df %>% select(AphiaID, Genus, Species, Latitude, Longitude)
colnames(plt_df) <- c("AphiaID", "Genus", "Species", "latitude", "longitude")
